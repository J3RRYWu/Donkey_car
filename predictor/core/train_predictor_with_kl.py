#!/usr/bin/env python3
"""
Training script for VAE Predictor with Classification + KL Divergence Loss

Total Loss = MSE(z_pred, z_target) + λ_cls * CrossEntropy + λ_kl * KL(z_pred || N(0,I))

The KL divergence term constrains predicted latents to stay within the 
VAE's learned distribution, preventing distribution shift that causes 
poor decoding quality.

Key Innovation:
- Forces LSTM to predict "in-distribution" latents
- Prevents the 100x MSE gap between VAE direct reconstruction (0.0004) 
  and LSTM+VAE prediction (0.04)
"""
...
def kl_divergence_latent(z_pred):
    """
    Compute KL divergence between predicted latent and standard normal prior
    
    KL(q(z) || p(z)) where p(z) = N(0, I)
    
    For a single sample z ~ q(z), assuming q(z) = N(z_pred, 0) (deterministic):
    KL ≈ 0.5 * ||z_pred||^2
    
    More accurately, we can treat z_pred as the mean of a Gaussian with 
    learned variance, but for simplicity we just regularize the L2 norm.
    
    Args:
        z_pred: (B, C, H, W) or (B, D) predicted latent
    
    Returns:
        kl_loss: scalar
    """
    # Flatten latent
    z_flat = z_pred.view(z_pred.size(0), -1)  # (B, D)
    
    # KL divergence to N(0, I)
    # If we assume the posterior is N(mu=z_pred, sigma^2=1), then:
    # KL(N(mu, 1) || N(0, 1)) = 0.5 * (mu^2 + 1 - 1 - log(1)) = 0.5 * mu^2
    kl = 0.5 * torch.sum(z_flat ** 2, dim=1)  # (B,)
    
    return kl.mean()


def train_epoch_with_kl(model, classifier, dataloader, optimizer, device, 
                        lambda_cls=0.5, lambda_kl=0.01, use_actions=False, 
                        teacher_forcing_prob=1.0):
    """
    Train one epoch with classification + KL divergence loss
    ...
    for batch in dataloader:
        ...
        # 3. KL Divergence Loss (NEW!)
        # Constrain predicted latents to stay in VAE's learned distribution
        # Apply to all predicted latents in the sequence
        z_pred_all = z_pred_seq.reshape(-1, *z_pred_seq.shape[2:])  # (B*T, C, H, W)
        kl_loss = kl_divergence_latent(z_pred_all)
        
        # Total loss
        loss = mse_loss + lambda_cls * cls_loss + lambda_kl * kl_loss
...
def validate_epoch_with_kl(model, classifier, dataloader, device, 
                           lambda_cls=0.5, lambda_kl=0.01, use_actions=False):
    """Validate with classification + KL loss"""
    model.eval()
    classifier.eval()
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # KL loss
            z_pred_all = z_pred_seq.reshape(-1, *z_pred_seq.shape[2:])
            kl_loss = kl_divergence_latent(z_pred_all)
...
def main():
    parser = argparse.ArgumentParser(description='Train VAE Predictor with Classification + KL Loss')
    ...
    # Output
    parser.add_argument('--save_dir', type=str, default='predictor/checkpoints_with_kl')
    ...
    print("="*70)
    print("Training VAE Predictor with Classification + KL Divergence Loss")
    print("="*70)
    print(f"Device: {device}")
    print(f"Lambda_cls: {args.lambda_cls}")
    print(f"Lambda_kl:  {args.lambda_kl} ★ NEW: Constrains latent distribution")
    ...
    train_history = {
        'loss': [], 'mse': [], 'cls': [], 'kl': [],
        'val_loss': [], 'val_mse': [], 'val_cls': [], 'val_kl': [], 'val_acc': []
    }
    
    for epoch in range(start_epoch + 1, args.epochs + 1):
        # Train
        train_stats = train_epoch_with_kl(
        ...
        # Validate
        val_stats = validate_epoch_with_kl(
        ...
        train_history['kl'].append(train_stats['kl'])
        ...
        train_history['val_kl'].append(val_stats['kl'])
        train_history['val_acc'].append(val_stats['accuracy'])
        
        print(f"Epoch {epoch}/{args.epochs}:")
        print(f"  Train - Loss: {train_stats['loss']:.4f}, MSE: {train_stats['mse']:.4f}, "
              f"Cls: {train_stats['cls']:.4f}, KL: {train_stats['kl']:.4f}")
        print(f"  Val   - Loss: {val_stats['loss']:.4f}, MSE: {val_stats['mse']:.4f}, "
              f"Cls: {val_stats['cls']:.4f}, KL: {val_stats['kl']:.4f}, Acc: {val_stats['accuracy']:.2f}%")
    ...
    print(f"\nKey Metrics:")
    print(f"  Final KL loss: {train_history['val_kl'][-1]:.4f}")
    print(f"  Final accuracy: {train_history['val_acc'][-1]:.2f}%")
